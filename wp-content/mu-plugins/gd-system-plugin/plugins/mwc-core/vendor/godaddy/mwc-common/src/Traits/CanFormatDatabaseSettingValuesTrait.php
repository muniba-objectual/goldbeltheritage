...
trait CanFormatDatabaseSettingValuesTrait
{
        ...
        switch ($setting->getType()) {
            ...
            case AbstractSetting::TYPE_BOOLEAN:
                throw new InvalidArgumentException(sprintf(
                    __('Please use %1$s or %2$s to format a boolean value for reading from or saving to storage.', 'mwc-common'),
    ...
    protected function stringToBool($value) : bool
    {
        ...
        throw new InvalidArgumentException(sprintf(
            __('Cannot handle a "%s" type to parse a valid boolean value.', 'mwc-common'),
            gettype($value)
        ));
    }
